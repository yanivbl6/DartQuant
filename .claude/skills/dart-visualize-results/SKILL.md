...
8. **~~Wildcard/variant pair-key auto-strip~~ — closed 2026-05-06.** Binary mode auto-strips tokens that are exclusive to one side of the comparison (e.g. `wrap` always-with-wAsym), but the same logic was gated to `mode == "binary"` — wildcard and variant modes hit "no matching pairs found" whenever the matched group carried a co-occurring token the baseline lacked. Concrete trigger: `-c "hws-*"` matches `hws-M4S4bmid` etc., but those runs always also have a `scalewise` token while vanilla runs don't, so pair-keys never aligned. Fix at [show_results.py:298](fake_quant/show_results.py#L298) — the `if mode == "binary"` block now runs for `("binary", "wildcard", "variant")` and pulls `baseline_indices` from `baseline_key` instead of `cmp_lower` (wildcard's baseline is `None`, not the compare expression).
