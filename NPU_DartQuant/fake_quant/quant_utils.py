    ...
    def forward(self, x):
        ...
        if self.online_full_had:
            ...
            if self.fp32_had:  # Full Hadamard in FP32
                x = hadamard_utils.matmul_hadU_npu(x.float(), self.had_K, self.K).to(x_dtype)
            else:  # Full Hadamard in FP16
                x = hadamard_utils.matmul_hadU_npu(x, self.had_K, self.K)
            ...
            if self.K == 1:
                ...
                import utils
                x = utils.hadamard_transform(x.reshape(-1, init_shape[-1] // self.had_dim, self.had_dim).transpose(1, 2),
