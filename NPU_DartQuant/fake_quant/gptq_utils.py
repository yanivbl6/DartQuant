...
class GPTQ:
    def __init__(self, layer):
        ...
        W = optimize_npu(layer.weight.data.clone())
        self.rows = W.shape[0]
    ...
    def fasterquant(
        ...
        W = optimize_npu(self.layer.weight.data.clone())
        W = W.float()
        ...
        Losses = torch.zeros_like(W, device=self.dev)  # Ensure Losses is on NPU
        Q = torch.zeros_like(W, device=self.dev)  # Ensure Q is on NPU
        ...
        for i1 in range(0, self.columns, blocksize):
            ...
            W1 = W[:, i1:i2].clone().to(self.dev)
            Q1 = torch.zeros_like(W1, device=self.dev)  # Ensure Q1 is on NPU
            Err1 = torch.zeros_like(W1, device=self.dev)  # Ensure Err1 is on NPU
            Losses1 = torch.zeros_like(W1, device=self.dev)  # Ensure Losses1 is on NPU
...
def gptq_fwrd(model, dataloader, dev, args):
    ...
    #     model.model.norm = model.model.norm.to(dev)
    # else:
    #     model.model.norm.load_state_dict(model.model.norm.state_dict())
    #     model.model.norm = model.model.norm.to(dev)
    ...
    outs = torch.zeros_like(inps, device=dev)
    ...
    for i in tqdm.tqdm(range(len(layers)), desc="(GPTQ Quant.) Layers"):
        ...
        for names in sequential:
            ...
            for j in range(args.nsamples):
                ...
                # print(f"layer device: {layer.device}, inps device: {inps[j].device}")
...
def rtn_fwrd(model, dev, args):
    ...
    for i in tqdm.tqdm(range(len(layers)), desc="(RtN Quant.) Layers"):
        ...
        for name in subset:
            ...
            quantizers['model.layers.%d.%s' % (i, name)] = quantizer.npu()
