...
# 执行命令并使用传入的参数
NPU_VISIBLE_DEVICES=${GPU_ID} python main_for_test.py \
    --model ${MODEL} \
    --fuse_norm \
    --use_r1 \
    --r1_path ${R1_PATH} \
    ...
    --save_qmodel_path /tmp/data/model/Meta-Llama-3-70B/ \
    2>&1 | tee -a experiments/${MODEL_NAME}_w${W_BITS}a${A_BITS}kv${KV_BITS}_w4a8_128nsamples.log
