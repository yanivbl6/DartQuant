...
import logging
import quant_utils
from hadamard_utils import apply_exact_had_to_linear, is_pow2
...
def fuse_ln_linear(layernorm: torch.nn.Module, linear_layers: typing.Iterable[torch.nn.Linear]) -> None:
    ...
    for linear in linear_layers:
        ...
        # print("linear", torch.sum(linear.weight.data, 0), torch.sum(linear.weight.data, 1))
        # exit()
...
def rotate_mlp_input(layer, Q, model_type, l_id, smooth):
    ...
    if smooth is not None:
        up_proj = layer.mlp.up_proj.to(device=utils.DEV, dtype=torch.float32)
        down_smooth = smooth[f'model.layers.{l_id}.mlp.down_smooth'].to(device=utils.DEV, dtype=torch.float32)
        up_proj.weight.data = torch.div(up_proj.weight.data.t(), down_smooth).t().to(device=utils.DEV, dtype=dtype)
...
def matmul_hadU_npu_had(X, hadK, transpose=False):
...
def rotate_faster_down_proj(layer, model_type, hardK):
    ...
    dtype = W.weight.data.dtype
    W.weight.data = matmul_hadU_npu_had(W.weight.data.float().npu(), hardK)
...
def rotate_head(model, Q: torch.Tensor) -> None:
    ...
    # Ensure that the matrix W is on the NPU and in the correct dtype (float32).
    dtype = W.weight.data.dtype
    W_ = W.weight.data.to(device=utils.DEV, dtype=torch.float32)
    # print(W_.shape, W_.dtype)
...
def rotate_ov_proj(layer, model_type, head_dim, kv_head,
    ...
    if 'offline' in use_r2:  # 获取 r2 并将其融入到模型中
        if r2_path is not None:
            ...
            Q = r2s[f"model.layers.{l_id}.self_attn.R2"].to(device=utils.DEV, dtype=torch.float32)
        else:
...
def apply_multi_head_rotate(module, Q, head_dim, l_id,
    ...
    W_ = W_.to(device=utils.DEV, dtype=torch.float32)
    ...
    if output:
        ...
        if smooth is not None:
            o_smooth = smooth['model.layers.{}.self_attn.o_smooth'.format(l_id)].to(device=utils.DEV, dtype=torch.float32)
        ...
        if smooth is not None:
            ...
            o_smooth = o_smooth.reshape(num_head, head_dim).to(device=utils.DEV, dtype=torch.float32)
            W_ = W_ * o_smooth.view(kv_head, 1, head_dim)
...
def rotate_model(model, args):
    if args.r1_path is not None:  # Q: r1 获取 r1 并将其融入到模型中
        Q = torch.load(args.r1_path, map_location=torch.device(utils.DEV))['R1'].to(dtype=torch.float32)
    else:
...
def register_online_rotation(module, Q: torch.Tensor):
    ...
    module.register_buffer('Q', Q.T.to(module.weight.data.device))  # Note F.linear(x, A) performs x@A.T
...
class QKRotationWrapper(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        # Ensure that q and k are on the correct device (NPU)
        q = q.to(device=utils.DEV, dtype=dtype)
        k = k.to(device=utils.DEV, dtype=dtype)
