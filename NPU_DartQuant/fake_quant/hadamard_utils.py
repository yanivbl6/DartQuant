...
def matmul_hadU_npu(X, hadK, K):
    # import fast_hadamard_transform
    import utils
    n = X.shape[-1]
    if K == 1:
        return utils.hadamard_transform(X.contiguous(), 1.0/torch.tensor(n).sqrt()) 
    # if transpose:
    #     hadK = hadK.T.contiguous()
    input = X.view(-1, K, n // K)
    input = utils.hadamard_transform(input.contiguous(), 1.0/torch.tensor(n).sqrt())
...
def apply_exact_had_to_linear(module, had_dim=-1, output=False):
    ...
    if had_dim == -1:
        if output:
            ...
            W_ = matmul_hadU_npu(W_.t(), had_K, K).t()
        else:
            had_K, K = get_hadK(in_features)
            W_ = matmul_hadU_npu(W_, had_K, K)
    else:
