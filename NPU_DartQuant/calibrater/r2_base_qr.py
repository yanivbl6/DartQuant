...
def train_r2(dataloader,
    ...
    if args.optim == 'sgd':
        optimizer = torch_npu.optim.NpuFusedSGD(R2.parameters(), momentum=args.mom, lr=args.lr)
        # optimizer = torch_npu.optim.NpuFusedLamb(R2.parameters(), lr=args.lr)
        # optimizer = torch.optim.SGD(R2.parameters(),
        #                             lr=args.lr, momentum=args.mom, foreach=False)
    elif args.optim == 'adam':
        optimizer = torch_npu.optim.NpuFusedAdam(R2.parameters(), lr=args.lr)
...
def main():
    ...
    for layer_id in range(args.hidden_layers):
        ...
        r2_dataset = R2Dataset(data_dire, args.nsamples, device)
        # 创建数据加载器
