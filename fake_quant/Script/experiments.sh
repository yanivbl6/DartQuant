...
./Script/dart_gptq_wxaykvz.sh baseline -g 2 -w 4 -a 8 -k 4 -G 128 --sym "$@" > ${RESULTS_DIR}/baseline_results.out 2> ${RESULTS_DIR}/baseline_results.err &
./Script/dart_gptq_wxaykvz.sh quarot -g 3 -w 4 -a 8 -k 4 -G 128 --sym "$@" > ${RESULTS_DIR}/quarot_results.out 2> ${RESULTS_DIR}/quarot_results.err &
./Script/dart_gptq_wxaykvz.sh dart -g 4 -w 4 -a 8 -k 4 -G 128 --sym "$@" > ${RESULTS_DIR}/dart_results.out 2> ${RESULTS_DIR}/dart_results.err &

# --- Static activation quantization runs (reuse GPTQ checkpoints from above) ---
./Script/dart_gptq_wxaykvz.sh baseline -g 5 -w 4 -a 8 -k 4 -G 128 --sym --static-act "$@" > ${RESULTS_DIR}/baseline_static_results.out 2> ${RESULTS_DIR}/baseline_static_results.err &
./Script/dart_gptq_wxaykvz.sh quarot -g 6 -w 4 -a 8 -k 4 -G 128 --sym --static-act "$@" > ${RESULTS_DIR}/quarot_static_results.out 2> ${RESULTS_DIR}/quarot_static_results.err &
./Script/dart_gptq_wxaykvz.sh dart -g 7 -w 4 -a 8 -k 4 -G 128 --sym --static-act "$@" > ${RESULTS_DIR}/dart_static_results.out 2> ${RESULTS_DIR}/dart_static_results.err &
